...
NSTimer *animTimer;
NSString * const devToken = @"S=s29:U=2f04ce:E=13f3513090a:C=137dd61dd0a:P=1cd:A=en-devtoken:H=870c3f166d4ab9d2cc6275d70664f84f";

@implementation EvernoteScreenshot

@synthesize noteStoreUri, noteStore, noteShareUri, currentScreenshotPaths, currentScreenshotExts;
...
- (void) connect {
    ...
    {      
        ...
        noteStoreUri = [[[NSURL alloc] initWithString:[userStore getNoteStoreUrl:[AppController getEvernoteAuthToken]]] autorelease];
        [EvernoteScreenshot sharedInstance].noteShareUri = [[[NSString alloc] initWithString: [[noteStoreUri absoluteString] stringByReplacingOccurrencesOfString:@"notestore" withString:@"sh"]] autorelease];
...
- (NSArray *) listNotebooks {   
    ...
    NSArray *notebooks = [[[NSArray alloc] initWithArray:[[self noteStore] listNotebooks:[AppController getEvernoteAuthToken]] ] autorelease];
...
- (EDAMNote *) getNote: (NSString *) guid {
    ...
    // Calling a function in the API
    return [noteStore getNote:[AppController getEvernoteAuthToken]:guid :true :true :true :true];
}
...
//    NSImage *imageA = [[[NSImage alloc] initWithContentsOfFile:imageAName] autorelease];
//    NSImage *imageB = [[[NSImage alloc] initWithContentsOfFile:imageBName] autorelease];
//    
...
- (NSArray *) generateNoteWrapper:(NSArray *)args{
    if(args && [args count] == 5){
        NSArray* retVals = [[EvernoteScreenshot sharedInstance] generateNote:(NSString *)[args objectAtIndex:0] :(NSString *)[args objectAtIndex:1] :(NSString *)[args objectAtIndex:2] :(NSString *)[args objectAtIndex:3] :(NSString *)[args objectAtIndex:4]];
        return retVals;
    }
    return nil;
}
...
//    [[EvernoteScreenshot sharedInstance] startAnimating];
    BOOL fileUploaded = false;
    BOOL songUploaded = false;
    AppDelegate *appDelegate = (AppDelegate *)[[NSApplication sharedApplication] delegate];
//    [[appDelegate getLocationManager] startUpdatingLocation];
//    NSUInteger tryTimeOut = 5;
//    while([[appDelegate getLocationManager] location] == nil && tryTimeOut != 0){
//        sleep(0.5);
//        tryTimeOut--;
//    }
//    
...
//    } else {
    ...
    for (NSString* path in currentScreenshotPaths){
        ...
        CGImageRef ref = QLThumbnailImageCreate(kCFAllocatorDefault, sourceUrl, albumArtSize, (CFDictionaryRef)options);
        if(ref){
            albumArtPath = [[NSString stringWithFormat:@"%@/albumArt_%d.png",[appDelegate getTmpPath],extIndex] stringByReplacingOccurrencesOfString:@" " withString:@""];
        ...
        if (imageNSData) {
            ...
            NSError *error;
            hash = [[NSString alloc] initWithContentsOfFile:[NSString stringWithFormat:@"%@/%@.md5", [appDelegate getTmpPath],[[path lastPathComponent] stringByDeletingPathExtension]] encoding:NSUTF8StringEncoding error:&error]; 
            ...
            EDAMData * imageData = [[EDAMData alloc] initWithBodyHash:[hash dataUsingEncoding: NSUTF8StringEncoding] size:[imageNSData length] body:imageNSData];
            ...
            if ([currentScreenshotPaths count] == 1 && [mimeType rangeOfString:@"image"].location != NSNotFound){
            ...
            if(albumArtPath && [markdownXHTML isEqualToString:@""]){
                if ([mimeType rangeOfString:@"image"].location == NSNotFound){
                    ...
                    albumArtHash = [[NSString alloc] initWithContentsOfFile:[NSString stringWithFormat:@"%@/albumArt_%d.md5", [appDelegate getTmpPath], extIndex] encoding:NSUTF8StringEncoding error:&error];
                    albumArtHash = [albumArtHash substringToIndex:[albumArtHash length] - 1];
                    
                    NSData *albumArtNSData= [[NSData alloc] 
                                             initWithContentsOfFile: albumArtPath];
                    
                    // 1) create the data EDAMData using the hash, the size and the data of the image
                    EDAMData * albumArtData = [[EDAMData alloc] initWithBodyHash:[albumArtHash dataUsingEncoding: NSUTF8StringEncoding] size:[albumArtNSData length] body:albumArtNSData];
                    
                    // 2) Create an EDAMResourceAttributes object with other important attributes of the file
                    EDAMResourceAttributes * albumArtAttributes = [[[EDAMResourceAttributes alloc] init] autorelease];
                    ...
                    EDAMResource* albumArtResource = [[[EDAMResource alloc]init]autorelease];
            ...
            if(! [ext isEqualToString: @"mp3"]){
                ...
                if ([mimeType rangeOfString:@"image"].location == NSNotFound &&
                    albumArtHash != nil && ![albumArtHash isEqualToString:@""] && [addPreview boolValue] && [markdownXHTML isEqualToString:@""]){
                ...
                if(title && ![title isEqualToString:@""]){
                    metaText = title; //[NSString stringWithFormat:@"%@<p>%@",imageENML, title];
                    if(artist && ![artist isEqualToString:@""]){
                        metaText = [NSString stringWithFormat:@"%@ by %@", metaText, artist];
                    }
                    if(album && ![album isEqualToString:@""]){
                        metaText = [NSString stringWithFormat:@"%@ off of %@", metaText, album];
                    }
                    metaText = [[[[[metaText stringByReplacingOccurrencesOfString: @"&" withString: @"&amp;"]
...
//        [appDelegate showLoading];
        ...
        // We now either want to create a notebook with the right name or get it
        //            if (notebookTitle == nil) notebookTitle = @"Snappi Screenshots";
        note.notebookGuid = [[EvernoteScreenshot sharedInstance] getOrCreateNotebook:notebookTitle];
        
        //            // Add geolocation tags to the note
        //            if ([appDelegate getLocationManager] != nil){
        //                CLLocation *currentLocation = [appDelegate getLocationManager].location;
        //                EDAMNoteAttributes *coordinates = [EDAMNoteAttributes alloc];
        //                coordinates.longitude = currentLocation.coordinate.longitude;
        //                coordinates.latitude  = currentLocation.coordinate.latitude;
        //                [note setAttributes:coordinates];
        //            }
        ...
        NSString *noteKey = [[EvernoteScreenshot sharedInstance] getShareKey:createdNote];
        shareLink = [NSString stringWithFormat:@"%@/%@", 
                     [NSString stringWithFormat:@"%@/%@", 
                      [[EvernoteScreenshot sharedInstance] noteShareUri], createdNote.guid], noteKey];
        ...
        NSMenuItem *item1 = [[NSMenuItem alloc] initWithTitle:@"Copy Shortlink"
                                                      action:@selector(statusItemClicked:) keyEquivalent:@""]; 
        [item1 setToolTip: [[[NSString alloc] initWithString: shareLink] autorelease]];
        [item1 setTarget: [EvernoteScreenshot sharedInstance]];
        NSMenuItem *item2 = [[NSMenuItem alloc] initWithTitle:@"View Stats"
                                                      action:@selector(statsItemClicked:) keyEquivalent:@""]; 
        [item2 setToolTip: [[[NSString alloc] initWithString: shareLink] autorelease]];
...
//        // send a tweet
//        if ([AppController getTwitterAuthToken] != nil){
//            [AppController sendTweet:shareLink];
//        }
        
//        [appDelegate hideLoading];
//        // if we haven't set the thumbnail, we need to generate a generic image
//        [appDelegate showComplete:thumb];
//        sleep(5);
//        [appDelegate hideComplete];
    }
    [fbc setShareLink:shareLink];
    NSArray *retVals = [NSArray arrayWithObjects:[NSNumber numberWithBool:fileUploaded], thumb, nil];
    return retVals;
}

// stop the animation of the statusbar and reset to default
// [[EvernoteScreenshot sharedInstance] stopAnimating];
// [[appDelegate getStatusItem] setImage:[[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"snappi_icon_g_16" ofType:@"png"]]];

// stop getting geolocation stats
//    [[EvernoteScreenshot sharedInstance] stopAnimating];

//    [[appDelegate getLocationManager] stopUpdatingLocation];
...
- (NSString *) getOrCreateNotebook:(NSString *) notebookName{
    ...
    @try {
        EDAMNotebook *test = [[EvernoteScreenshot sharedInstance] createNotebook:notebook];
        nguid = [test guid];
    }
    @catch ( NSException *e ) {
...
- (void) statusItemClicked:(NSMenuItem *) menuItem{
    NSLog(@"CLICKED");
    // NSURL *u = [[NSURL alloc] initWithString:[menuItem toolTip]];
...
- (NSTask *) createHash:(NSString *) path {
    ...
    NSArray	*args = [NSArray arrayWithObjects: @"-c",
                    [NSString stringWithFormat:@"md5 \"%@\" | awk -F'= ' '{ print $2 }' > \"%@/%@.md5\"",
                     escPath, [appDelegate getTmpPath], [[escPath lastPathComponent] stringByDeletingPathExtension] ],
                     nil];
    [task setArguments: args];
